...
// Konstanta yang sebelumnya hilang
...
export default function LandingPage() {
  ...
  // State yang sebelumnya hilang
  const [expandedCard, setExpandedCard] = useState<'ai' | 'community' | null>(null);
    ...
    <div className="relative min-h-screen overflow-hidden">
        ...
        <div className={`mx-auto px-6 py-3.5 rounded-2xl backdrop-blur-2xl border border-white/10 bg-[#0A0A0A]/60 shadow-2xl shadow-black/50 transition-all duration-500 ${scrolled ? 'bg-[#0A0A0A]/80 border-brand-500/20' : ''}`}>
          <div className="flex items-center justify-between gap-6">
            ...
            <Link href="/" className="flex items-center gap-2.5 group shrink-0">
              <div className="relative w-9 h-9 rounded-xl bg-gradient-to-br from-brand-500 to-brand-400 flex items-center justify-center shadow-lg shadow-brand-500/30 group-hover:shadow-brand-500/50 transition-all duration-300 group-hover:scale-110">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {isAuthenticated ? (
                <Link href="/dashboard" className="cursor-target px-5 py-2.5 rounded-xl bg-gradient-to-r from-brand-500 to-brand-400 text-white text-sm font-semibold hover:shadow-lg hover:shadow-brand-500/40 transition-all duration-300 hover:-translate-y-0.5">
                  Dashboard
                </Link>
              ) : (
                <>
                  ...
                  <Link href="/register" className="cursor-target px-5 py-2.5 rounded-xl bg-gradient-to-r from-brand-500 to-brand-400 text-white text-sm font-semibold hover:shadow-lg hover:shadow-brand-500/40 transition-all duration-300 hover:-translate-y-0.5 border border-white/10">
                    Daftar Gratis
                  </Link>
                </>
              )}
          ...
          <div className="flex flex-col gap-2">
            {[
              ...
              <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)}
                className={`px-4 py-3 rounded-xl text-sm font-medium transition-all duration-300 ${item.primary ? 'bg-gradient-to-r from-brand-500 to-brand-400 text-white text-center' : 'text-white/70 hover:text-white hover:bg-white/5'}`}
              >
                {item.label}
              </Link>
            ))}
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 px-6 overflow-hidden">
        <div className={`relative z-10 max-w-5xl mx-auto text-center transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/register"
              className="cursor-target group relative px-8 py-3.5 rounded-2xl bg-gradient-to-r from-brand-500 to-brand-400 text-white font-semibold text-lg hover:shadow-2xl hover:shadow-brand-500/30 transition-all duration-500 hover:-translate-y-1"
            >
              Mulai Evaluasi — Gratis
              <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-brand-400 to-purple-400 opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10 blur-xl" />
            </Link>
            <a
              href="#live-preview"
      ...
      <section id="features" className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-4 h-auto md:h-[500px]">
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-br from-brand-500/5 to-brand-400/5 opacity-0 group-hover:opacity-100 transition-opacity" />
              <div className={`p-8 w-full h-full flex flex-col transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] ${expandedCard === 'ai' ? 'items-start' : 'items-center justify-center text-center'}`}>
                <div className={`rounded-2xl bg-gradient-to-br from-brand-500 to-brand-400 flex items-center justify-center shrink-0 transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] shadow-lg ${expandedCard === 'ai' ? 'w-16 h-16 mb-8' : 'w-20 h-20 mb-4'}`}>
                ...
                <div className={`transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] overflow-hidden w-full ${expandedCard === 'ai' ? 'opacity-100 max-h-96' : 'opacity-0 max-h-0 md:max-h-0'}`}>
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-br from-brand-400/5 to-brand-300/5 opacity-0 group-hover:opacity-100 transition-opacity" />
              <div className={`p-8 w-full h-full flex flex-col transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] ${expandedCard === 'community' ? 'items-start' : 'items-center justify-center text-center'}`}>
                <div className={`rounded-2xl bg-gradient-to-br from-brand-400 to-brand-300 flex items-center justify-center shrink-0 transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] shadow-lg ${expandedCard === 'community' ? 'w-16 h-16 mb-8' : 'w-20 h-20 mb-4'}`}>
                ...
                <div className={`transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] overflow-hidden w-full ${expandedCard === 'community' ? 'opacity-100 max-h-96' : 'opacity-0 max-h-0 md:max-h-0'}`}>
                  <p className="text-text-secondary leading-relaxed mb-6 text-lg max-w-2xl">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            ...
            <ScrollReveal direction="left" delay={0}>
              <div className="browser-frame glow-brand shadow-2xl shadow-brand-500/10 rounded-xl overflow-hidden border border-white/10">
                {/* Title bar */}
                <div className="browser-frame-bar flex items-center px-4 py-3 bg-white/5 border-b border-white/10 gap-2">
                  <span className="w-3 h-3 rounded-full bg-red-500" />
                  <span className="w-3 h-3 rounded-full bg-yellow-400" />
                  <span className="w-3 h-3 rounded-full bg-green-500" />
                  <div className="flex-1 mx-4 h-7 rounded-md bg-black/20 flex items-center px-3 gap-2 border border-white/5">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-white/30">
                      <circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/>
                    </svg>
                    <span className="text-xs text-white/40 truncate">https://example-website.com</span>
                  </div>
                </div>
                ...
                <div className="relative aspect-video bg-gradient-to-br from-slate-800 to-slate-900 overflow-hidden">
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col gap-4">
                    {/* Navbar sim */}
                    <div className="h-10 bg-white/5 rounded-lg flex items-center px-4 gap-3 border border-white/5">
                      <div className="w-20 h-4 bg-brand-500/40 rounded" />
                      <div className="flex-1" />
                      {[1,2,3].map(i => <div key={i} className="w-12 h-2.5 bg-white/10 rounded" />)}
                      <div className="w-20 h-7 bg-brand-500/30 rounded-md" />
                    </div>
                    {/* Hero sim */}
                    <div className="flex-1 flex flex-col gap-3 items-center justify-center">
                      <div className="w-48 h-5 bg-white/20 rounded" />
                      <div className="w-72 h-8 bg-white/30 rounded" />
                      <div className="w-56 h-5 bg-white/15 rounded" />
                      <div className="flex gap-3 mt-3">
                        <div className="w-24 h-9 bg-brand-500/50 rounded-lg" />
                        <div className="w-24 h-9 bg-white/10 rounded-lg" />
                      </div>
                    </div>
                    {/* Cards sim */}
                    <div className="grid grid-cols-3 gap-3">
                      {[1,2,3].map(i => <div key={i} className="h-16 bg-white/5 rounded-lg border border-white/5" />)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {PREVIEW_PINS.map((pin) => (
                  <div
                    ...
                    className={`glass-card px-4 py-4 flex items-center gap-4 transition-all duration-300 cursor-pointer border border-transparent ${
                      activePin === pin.id ? 'border-opacity-60 scale-[1.02] bg-white/5' : 'hover:bg-white/5'
                    }`}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-sm font-bold" style={{ color: pin.score >= 80 ? '#4ade80' : pin.score >= 60 ? '#facc15' : '#f87171' }}>
                          {pin.score}/100
                        </span>
                      </div>
                ...
                <div className="glass-card px-5 py-5 border-brand-500/20 mt-6 bg-brand-500/5">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-xs text-text-tertiary mb-1 uppercase tracking-wider">Overall Score</div>
                      <div className="text-4xl font-bold gradient-text">79/100</div>
                    </div>
                    <div className="text-right">
                      <div className="text-xs text-text-tertiary mb-1 uppercase tracking-wider">Status</div>
                      <span className="px-4 py-1.5 rounded-full bg-yellow-500/10 text-yellow-400 text-sm font-semibold border border-yellow-500/20">Cukup Baik</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-6xl mx-auto">
          <ScrollReveal direction="up">
            <div className="text-center mb-16">
              <div className="section-divider" />
              <h2 className="text-3xl md:text-5xl font-bold text-text-primary mb-4">
                Cara Kerja <span className="gradient-text">Platform</span>
              </h2>
              <p className="text-text-secondary text-lg max-w-2xl mx-auto">
                Tiga langkah sederhana untuk mendapatkan feedback desain yang actionable
              </p>
            </div>
          </ScrollReveal>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <ScrollReveal key={item.step} delay={i * 150} direction="up">
                <div className="relative text-center group p-6 rounded-2xl hover:bg-white/5 transition-all duration-500">
                  <div className="w-20 h-20 rounded-3xl bg-surface-100 border border-border mx-auto mb-6 flex items-center justify-center text-4xl group-hover:border-brand-500/30 group-hover:-translate-y-2 group-hover:shadow-xl group-hover:shadow-brand-500/10 transition-all duration-500 bg-white/5">
                    {item.icon}
                  </div>
                  <div className="absolute top-6 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-brand-500 text-white flex items-center justify-center text-sm font-bold border-4 border-[#0F172A]">
                    {item.step}
                  </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {[
              ...
              <ScrollReveal key={cat.key} delay={i * 80} direction="up">
                <div className="glass-card p-5 hover:-translate-y-1.5 hover:border-brand-500/20 transition-all duration-300 group h-full border border-white/5">
                  <div
                  ...
                  <div className="mt-4 h-1.5 rounded-full" style={{ background: cat.color + '30' }}>
      ...
      <section className="py-24 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <ScrollReveal direction="up">
            <div className="glass-card p-12 md:p-16 relative overflow-hidden glow-brand border border-white/10">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Link
                    href="/register"
                    className="cursor-target inline-block px-8 py-4 rounded-2xl bg-gradient-to-r from-brand-500 to-brand-400 text-white font-semibold text-lg hover:shadow-2xl hover:shadow-brand-500/30 transition-all duration-500 hover:-translate-y-1"
                  >
      ...
      {/* ═══════════════════════════════ FOOTER ══════════════════════════════ */}
      <footer className="bg-[#0A0A0A] border-t border-white/10 pt-24 pb-8 px-6 relative overflow-hidden">
        ...
        <div className="absolute -bottom-4 md:-bottom-8 left-0 w-full overflow-hidden flex justify-center pointer-events-none select-none z-0">
          <span className="text-[20vw] font-black leading-none text-white/[0.02] tracking-tighter">
            DesignLens
          </span>
        </div>
      </footer>
